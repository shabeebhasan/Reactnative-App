...
export const verifyCode = createAsyncThunk(
  "login/verifyCode",
  async (payload) => {
    const response = await api.verifyCodeRequest(payload);
    return response.data;
  }
);
...
export const setPasswordRequest = createAsyncThunk(
  "login/setPassword",
  async (payload) => {
    const response = await api.apiSetPasswordRequest(payload);
    return response.data;
  }
);
...
export const slice = createSlice({
  ...
  extraReducers: {
    ...
    [setPasswordRequest.pending]: (state) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending";
        state.api.error = null;
      }
    },
    [setPasswordRequest.fulfilled]: (state, action) => {
      if (state.api.loading === "pending") {
        state.user = action.payload;
        state.api.loading = "idle";
        state.api.error = null;
    ...
    [setPasswordRequest.rejected]: (state, action) => {
      if (state.api.loading === "pending") {
        state.api.error = mapErrorMessage(action.error);
        state.api.loading = "idle";
      }
